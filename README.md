...
1. **가짜 뉴스 저장 이벤트 수신**  
   - 가짜 뉴스가 DB에 저장되면, 저장 완료 이벤트가 비동기 방식으로 퀴즈 생성 서비스에 전달됨  

2. **퀴즈 출제 데이터 준비**  
   - 이벤트로 전달된 가짜 뉴스 ID를 통해 관련 뉴스 내용 조회  
   - 진짜 뉴스와 가짜 뉴스 간 핵심 정보를 비교 및 분석  

3. **퀴즈 문제 자동 생성**  
   - **OX 퀴즈**: 가짜 뉴스의 핵심 문장이나 사실 여부를 기반으로 참/거짓 문제 생성  
   - **디테일 퀴즈**: 뉴스 내용의 세부 정보(예: 인물, 날짜, 장소 등)를 묻는 객관식 문제 생성  
   - AI 모델과 규칙 기반 로직이 결합되어 자동으로 문제와 정답 생성  
